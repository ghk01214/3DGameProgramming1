...
{
	// Instance 정보를 저장할 Vertex 버퍼를 업로드 Heap 유형으로 생성
	...
	// Vertex 버퍼(업로드 Heap)에 대한 포인터 저장
	m_pd3dcbGameObjects->Map(0, NULL, (void**)&m_pcbMappedGameObjects);
}

// Instancing 정보(객체의 월드 변환 행렬과 색상)를 Vertex 버퍼에 복사
...
{
	// x축, y축, z축 양의 방향의 객체 개수
	// 각 값을 1씩 늘리거나 줄이면서 실행할 때 Frame 레이트가 어떻게 변하는 가를 살펴보기 바란다.
	INT xObjects = 10;
	INT yObjects = 10;
	INT zObjects = 10;
	INT i = 0;

	// x축, y축, z축으로 21개씩 총 21 x 21 x 21 = 9261개의 정육면체를 생성 후 배치
	...
	for (INT x = -xObjects; x <= xObjects; ++x)
	{
		for (INT y = -yObjects; y <= yObjects; ++y)
		{
			for (INT z = -zObjects; z <= zObjects; ++z)
			{
				...
				pRotatingObject->SetPosition(fxPitch * x, fyPitch * y, fzPitch * z);		//각 정육면체 객체의 위치 설정
	...
	// 가로x세로x높이가 12x12x12인 정육면체 Mesh 생성
	// Instancing을 사용하여 Rendering하기 위하여 하나의 Game Object만 Mesh를 가진다.
	CCubeMeshDiffused* pCubeMesh{ new CCubeMeshDiffused(pd3dDevice, pd3dCommandList, 12.0f, 12.0f, 12.0f) };
	m_ppObjects[0]->SetMesh(pCubeMesh);

	// Instancing을 위한 버퍼(Structured Buffer) 생성
...
void CInstancingShader::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	// 하나의 Vertex 데이터를 사용하여 모든 Game Object(Instance)들 Rendering
