...
/*플레이어의 위치를 변경하는 함수.플레이어의 위치는 기본적으로 사용자가 플레이어를 이동하기 위한 키를 누를 때 변경.
...
{
	// bUpdateVelocity가 TRUE이면 플레이어를 이동하지 않고 속도 벡터를 변경
	if (bVelocity)
	{
...
{
	DWORD dwCameraMode{ m_pCamera->GetMode() };

	// 1인칭 카메라 또는 3인칭 카메라의 경우 플레이어의 회전은 약간의 제약이 따른다.
	if ((dwCameraMode == FIRST_PERSON_CAMERA) || (dwCameraMode == THIRD_PERSON_CAMERA))
	{
		...
		+89도 보다 크거나 -89도 보다 작으면 m_fPitch가 +89도 또는 -89도가 되도록 회전각도(x) 수정.*/
		if (x != 0.0f)
		{
			m_fPitch += x;
		...
		if (z != 0.0f)
		{
			...
			m_fRoll이 +20도 또는 -20도가 되도록 회전각도(z) 수정.*/
			m_fRoll += z;
		...
		플레이어의 로컬 y축(Up 벡터)을 기준으로 로컬 z축(Look 벡터)과 로컬 x축(Right 벡터)을 회전.
	...
	else if (dwCameraMode == SPACESHIP_CAMERA)
	{
		// Spaceship 카메라에서 플레이어의 회전은 회전 각도의 제한이 없다. 그리고 모든 축을 중심으로 회전 가능
		m_pCamera->Rotate(x, y, z);
	...
	// 회전으로 인해 플레이어의 로컬 x축, y축, z축이 서로 직교하지 않을 수 있으므로 z축(Look 벡터)을 기준으로 하여 서로 직교하고 단위벡터가 되도록 한다.
	m_xmf3Look	 = Vector3::Normalize(m_xmf3Look);
	m_xmf3Right	 = Vector3::CrossProduct(m_xmf3Up, m_xmf3Look, TRUE);
	m_xmf3Up	 = Vector3::CrossProduct(m_xmf3Look, m_xmf3Right, TRUE);
}

// 이 함수는 매 Frame 호출된다.플레이어의 속도 벡터에 중력과 마찰력 등을 적용한다.
void CPlayer::Update(FLOAT fTimeElapsed)
{
	// 플레이어의 속도 벡터를 중력 벡터와 더한다. 중력 벡터에 fTimeElapsed를 곱하는 것은 중력을 시간에 비례하도록 적용한다는 의미.
	m_xmf3Velocity = Vector3::Add(m_xmf3Velocity, Vector3::ScalarProduct(m_xmf3Gravity, fTimeElapsed, FALSE));

	// 플레이어의 속도 벡터의 XZ성분의 크기를 구한다. 이것이 XZ평면의 최대 속력보다 크면 속도 벡터의 x와 z방향 성분 조정.
	...
	// 플레이어를 속도 벡터 만큼 실제로 이동(카메라도 이동될 것)
	XMFLOAT3 xmf3Velocity{ Vector3::ScalarProduct(m_xmf3Velocity, fTimeElapsed, FALSE) };
	
	Move(xmf3Velocity, FALSE);

	/*플레이어의 위치가 변경될 때 추가로 수행할 작업을 수행.
	...
	// 카메라가 3인칭 카메라이면 카메라가 변경된 플레이어 위치를 바라보도록 설정
	if (dwCameraMode == THIRD_PERSON_CAMERA)
...
CCamera* CPlayer::OnChangeCamera(DWORD dwNewCameraMode, DWORD dwCurrentCameraMode)
{
	...
	/*현재 카메라의 모드가 Spaceship 모드이고 새로운 카메라가 1인칭 또는 3인칭 카메라이면
	플레이어의 Up 벡터를 월드 좌표계의 y축 방향 벡터(0, 1, 0)이 되도록 한다. 즉, 똑바로 서도록 한다.
	그리고 Spaceship 카메라의 경우 플레이어의 이동에는 제약이 없다. 특히, y축 방향의 움직임이 자유롭다.
	그러므로 플레이어의 위치는 공중(위치 벡터의 y좌표가 0보다 크다)이 될 수 있다.
	...
	if (dwCurrentCameraMode == SPACESHIP_CAMERA)
	{
		...
		m_fPitch		 = 0.0f;
		m_fRoll			 = 0.0f;
		// Look 벡터와 월드 좌표계의 z축(0, 0, 1)이 이루는 각도(내적=cos)를 계산하여 플레이어의 y축의 회전 각도 m_fYaw로 설정
	...
	{
		// 새로운 카메라의 모드가 Spaceship 모드이고 현재 카메라 모드가 1인칭 또는 3인칭 카메라이면 플레이어의 로컬 축을 현재 카메라의 로컬 축과 같게 만든다
...
CCamera* CAirplanePlayer::ChangeCamera(DWORD nNewCameraMode, FLOAT fTimeElapsed)
{
	...
	case FIRST_PERSON_CAMERA:
	{
		// 플레이어의 특성을 1인칭 카메라 모드에 맞게 변경(중력은 미적용)
		SetFriction(200.0f);
	...
	case SPACESHIP_CAMERA:
	{
		// 플레이어의 특성을 Spaceship 카메라 모드에 맞게 변경(중력은 미적용)
		SetFriction(125.0f);
		SetGravity(XMFLOAT3(0.0f, 0.0f, 0.0f));
	...
	case THIRD_PERSON_CAMERA:
	{
		...
		// 3인칭 카메라의 지연 효과 설정. 값을 0.25f 대신에 0.0f와 1.0f로 설정한 결과를 비교하기 바란다.
...
CCamera* CTerrainPlayer::ChangeCamera(DWORD nNewCameraMode, FLOAT fTimeElapsed)
{
	...
	{
		SetFriction(250.0f);
		// 1인칭 카메라일 때 플레이어에 y축 방향으로 중력 작용
	...
	{
		SetFriction(125.0f);
		// Spaceship 카메라일 때 플레이어에 중력 작용 X
		SetGravity(XMFLOAT3(0.0f, 0.0f, 0.0f));
		SetMaxVelocityXZ(300.0f);
		SetMaxVelocityY(400.0f);
	...
	{
		SetFriction(250.0f);
		// 3인칭 카메라일 때 플레이어에 y축 방향으로 중력 작용
		SetGravity(XMFLOAT3(0.0f, -250.0f, 0.0f));
		SetMaxVelocityXZ(300.0f);
		SetMaxVelocityY(400.0f);
