...
void CSpaceShipCamera::Rotate(FLOAT fPitch, FLOAT fYaw, FLOAT fRoll)
{
	...
	{
		// 플레이어의 로컬 x축에 대한 x 각도의 회전 행렬 계산
		...
		// 카메라의 위치 벡터에서 플레이어의 위치 벡터를 뺀다. 결과 = 플레이어 위치를 기준(원점)으로 한 카메라의 위치 벡터
		m_xmf3Position = Vector3::Subtract(m_xmf3Position, m_pPlayer->GetPosition());
		// 플레이어의 위치를 중심으로 카메라의 위치 벡터(플레이어를 기준으로 한) 회전
		m_xmf3Position = Vector3::TransformCoord(m_xmf3Position, xmmtxRotate);
		// 회전시킨 카메라의 위치 벡터에 플레이어의 위치를 더하여 카메라의 위치 벡터 계산
...
CFirstPersonCamera::CFirstPersonCamera(CCamera* pCamera)
{
	...
	if (pCamera)
	{
		...
		월드좌표의 y축이 되도록, 즉, 똑바로 서있는 형태로 설정한다는 의미이다.
...
void CFirstPersonCamera::Rotate(FLOAT fPitch, FLOAT fYaw, FLOAT fRoll)
{
	if (fPitch != 0.0f)
	{
		// 카메라의 로컬 x축을 기준으로 회전하는 행렬 생성. 사람의 경우 고개를 끄떡이는 동작.
