...
// 카메라의 종류(모드: Mode)를 나타내는 상수 선언
#define FIRST_PERSON_CAMERA	 0x01
#define SPACESHIP_CAMERA	 0x02
#define THIRD_PERSON_CAMERA	 0x03

// Frame 버퍼 크기와 Aspect Ratio(종횡비)를 나타내는 상수 선언
...
public:
	...
	virtual ~CCamera();
public:
	// 카메라의 정보를 Shader 프로그램에 전달하기 위한 상수 버퍼 생성 및 갱신
...
public:
	...
	/*카메라가 여러번 회전을 하게 되면 누적된 실수 연산의 부정확성 때문에 카메라의 로컬 x축(Right), y축(Up), z축(Look)이 서로 직교하지 않을 수 있다.
	카메라의 로컬 x축(Right), y축(Up), z축(Look)이 서로 직교하도록 만들	어준다.*/
...
protected:
	FLOAT				 m_fPitch;							// 카메라의 x축(Right) 회전각
	FLOAT				 m_fRoll;							// 카메라의 y축(Up) 회전각
	FLOAT				 m_fYaw;							// 카메라의 z축(Look) 회전각
protected:
	DWORD				 m_nMode;							// 카메라의 종류(1인칭 카메라, 스페이스-쉽 카메라, 3인칭 카메라)
protected:
...
protected:
	CPlayer*			 m_pPlayer{ nullptr };				// 카메라를 가지고 있는 플레이어에 대한 포인터
};
