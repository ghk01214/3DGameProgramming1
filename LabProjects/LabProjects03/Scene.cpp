...
CGameObject *CScene::PickObjectPointedByCursor(int xClient, int yClient, CCamera *pCamera)
{
	...
	xmf3PickPosition.x = (((2.0f * xClient) / pCamera->d3dViewport.Width) - 1) / pCamera->xmf4x4Projection._11;
	xmf3PickPosition.y = -(((2.0f * yClient) / pCamera->d3dViewport.Height) - 1) / pCamera->xmf4x4Projection._22;
	xmf3PickPosition.z = 1.0f;

	XMVECTOR xmvPickPosition = XMLoadFloat3(&xmf3PickPosition);
	XMMATRIX xmmtxView = XMLoadFloat4x4(&pCamera->xmf4x4View);
...
void CScene::CheckObjectByBulletCollisions()
{
	...
	for (int i = 0; i < m_nObjects; i++)
	{
		...
		{
			if (ppBullets[j]->bActive && m_ppObjects[i]->m_xmOOBB.Intersects(ppBullets[j]->m_xmOOBB)) 
			{
				CExplosiveObject *pExplosiveObject = (CExplosiveObject *)m_ppObjects[i];
				pExplosiveObject->m_bBlowingUp = true;
				ppBullets[j]->bActive = false;
			}
		}
	}
}
