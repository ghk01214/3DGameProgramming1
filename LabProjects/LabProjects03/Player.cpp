...
{
	this->m_xmf3CameraOffset = xmf3CameraOffset;
	m_pCamera->SetLookAt(Vector3::Add(m_xmf3Position, this->m_xmf3CameraOffset), m_xmf3Position, m_xmf3Up);
...
void CAirplanePlayer::Animate(FLOAT fElapsedTime)
{
	...
	for (INT i{ 0 }; i < BULLETS; i++)
	{
		if (m_ppBullets[i]->bActive) m_ppBullets[i]->Animate(fElapsedTime);
	}
}
...
void CAirplanePlayer::Render(HDC hDCFrameBuffer, CCamera *pCamera)
{
	...
	for (INT i{ 0 }; i < BULLETS; i++) if (m_ppBullets[i]->bActive) m_ppBullets[i]->Render(hDCFrameBuffer, pCamera);
}
