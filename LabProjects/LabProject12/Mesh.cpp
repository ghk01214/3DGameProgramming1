...
void CMesh::Render(ID3D12GraphicsCommandList* pd3dCommandList, UINT nInstances)
{
	...
	if (m_pd3dIndexBuffer)
	{
		...
		pd3dCommandList->DrawInstanced(m_nVertices, nInstances, m_nOffset, 0);			// Mesh의 Vertex 버퍼 뷰를 Rendering(Pipeline(입력 조립기)을 작동하게 한다)
}
...
{
	// Vertex 버퍼 뷰와 Instancing 버퍼 뷰를 입력-조립 단계에 설정
...
CAirplaneMeshDiffused::CAirplaneMeshDiffused(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, FLOAT fWidth, FLOAT fHeight, FLOAT fDepth, XMFLOAT4 xmf4Color)
{
	...
	// 비행기 Mesh를 표현하기 위한 정점 데이터
	CDiffusedVertex pVertices[24 * 3];
	FLOAT x1{ fx * 0.2f }, x2{ fx * 0.1f };
	FLOAT y1{ fy * 0.2f }, y3{ fy * 0.3f }, y2{ ((y1 - (fy - y3)) / x1) * x2 + (fy - y3) };
	INT i{ 0 };
