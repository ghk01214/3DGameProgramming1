...
/*플레이어의 위치를 변경하는 함수. 플레이어의 위치는 기본적으로 사용자가 플레이어를 이동하기 위한 키보드를 누를 때 변경.
...
void CPlayer::Move(DWORD dwDirection, FLOAT fDistance, BOOL bVelocity)
{
	if (dwDirection)
	{
		...
		// ‘→’를 누르면 로컬 x축 방향으로 이동(우향)
		if (dwDirection & DIR_RIGHT)
			xmf3Shift = Vector3::Add(xmf3Shift, m_xmf3Right, fDistance);

		// ‘←’를 누르면 로컬 x축 방향으로 이동(좌향)
		if (dwDirection & DIR_LEFT)
			xmf3Shift = Vector3::Add(xmf3Shift, m_xmf3Right, -fDistance);

		// ‘Page Up’을 누르면 로컬 y축 방향으로 이동(상승)
		if (dwDirection & DIR_UP)
			xmf3Shift = Vector3::Add(xmf3Shift, m_xmf3Up, fDistance);

		// ‘Page Down’을 누르면 로컬 y축 방향으로 이동(하강)
...
void CPlayer::Move(XMFLOAT3& xmf3Shift, BOOL bVelocity)
{
	// bVelocity가 TRUE이면 플레이어를 이동하지 않고 속도 벡터 변경
	if (bVelocity)
	{
...
void CPlayer::Rotate(FLOAT x, FLOAT y, FLOAT z)
{
	...
	if ((nCameraMode == FIRST_PERSON_CAMERA) || (nCameraMode == THIRD_PERSON_CAMERA))
	{
		...
		그러므로 x축을 중심으로 회전하는 각도는 -89.0 ~ +89.0도 사이로 제한.
		...
		/*플레이어를 회전. 1인칭 카메라 또는 3인칭 카메라에서 플레이어의 회전은 로컬 y축에서만 일어난다.
		플레이어의 로컬 y축(Up 벡터)을 기준으로 로컬 z축(Look 벡터)과 로컬 x축(Right 벡터)을 회전시킨다.
	...
	{
		// Spaceship 카메라에서 플레이어의 회전은 회전 각도의 제한이 없다. 모든 축을 중심으로 회전 가능
		m_pCamera->Rotate(x, y, z);
	...
	/*회전으로 인해 플레이어의 로컬 x축, y축, z축이 서로 직교하지 않을 수 있으므로 z축(Look 벡터)을
	기준으로 하여 서로 직교하고 단위벡터가 되도록 한다.*/
	m_xmf3Look	 = Vector3::Normalize(m_xmf3Look);
	m_xmf3Right	 = Vector3::CrossProduct(m_xmf3Up, m_xmf3Look, TRUE);
	m_xmf3Up	 = Vector3::CrossProduct(m_xmf3Look, m_xmf3Right, TRUE);
}

// 이 함수는 매 프레임마다 호출. 플레이어의 속도 벡터에 중력과 마찰력 등을 적용.
void CPlayer::Update(FLOAT fTimeElapsed)
{
	// 플레이어의 속도 벡터를 중력 벡터와 더한다. 중력 벡터에 fTimeElapsed를 곱하는 것은 중력을 시간에 비례하도록 적용한다는 의미
	m_xmf3Velocity = Vector3::Add(m_xmf3Velocity, Vector3::ScalarProduct(m_xmf3Gravity, fTimeElapsed, FALSE));

	// 플레이어의 속도 벡터의 xz성분의 크기를 구한다. 이것이 xz평면의 최대 속력보다 크면 속도 벡터의 x와 z방향 성분을 조정
	...
	// 플레이어의 속도 벡터의 y성분의 크기를 구한다. 이것이 y축 방향의 최대 속력보다 크면 속도 벡터의 y방향 성분을 조정
	...
	// 플레이어를 속도 벡터 만큼 실제로 이동(카메라도 이동)
	XMFLOAT3 xmf3Velocity{ Vector3::ScalarProduct(m_xmf3Velocity, fTimeElapsed, FALSE) };

	Move(xmf3Velocity, FALSE);

	/*플레이어의 위치가 변경될 때 추가로 수행할 작업을 수행. 플레이어의 새로운 위치가 유효한 위치가 아닐 수도 있고
	...
	// 플레이어의 위치가 변경되었으므로 3인칭 카메라 갱신
	if (nCameraMode == THIRD_PERSON_CAMERA)
		m_pCamera->Update(m_xmf3Position, fTimeElapsed);

	// 카메라의 위치가 변경될 때 추가로 수행할 작업 수행
...
// 카메라를 변경할 때 ChangeCamera() 함수에서 호출되는 함수이다. nCurrentCameraMode = 현재 카메라의 모드, dwNewCameraMode = 새로 설정할 카메라 모드
CCamera* CPlayer::ChangeCamera(DWORD dwNewCameraMode, DWORD nCurrentCameraMode)
{
	// 새로운 카메라의 모드에 따라 카메라 새로 생성
	...
	/*현재 카메라의 모드가 Spaceship 모드의 카메라이고 새로운 카메라가 1인칭 또는 3인칭 카메라이면 플레이어의
	Up 벡터를 월드좌표계의 y축 방향 벡터(0, 1, 0)이 되도록 한다. 즉, 똑바로 서도록 한다.
	그리고 Spaceship 카메라의 경우 플레이어의 이동에는 제약이 없다. 특히, y축 방향의 움직임이 자유롭다.
	...
	if (nCurrentCameraMode == SPACESHIP_CAMERA)
	{
		...
		m_fPitch	 = 0.0f;
		m_fRoll		 = 0.0f;
		// Look 벡터와 월드좌표계의 z축(0, 0, 1)이 이루는 각도(내적=cos)를 계산하여 플레이어의 y축의 회전 각도 m_fYaw로 설정
	...
	{
		/*새로운 카메라의 모드가 Spaceship 모드의 카메라이고 현재 카메라 모드가 1인칭 또는 3인칭 카메라이면
...
/*플레이어의 위치와 회전축으로부터 월드 변환 행렬을 생성하는 함수. 플레이어의 Right 벡터가 월드 변환 행렬의 첫 번째 행 벡터,
...
void CPlayer::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	// 카메라 모드가 3인칭이면 플레이어 객체 Rendering
	if (nCameraMode == THIRD_PERSON_CAMERA)
	{
		if (m_pShader)
...
// 카메라를 변경할 때 호출되는 함수. dwNewCameraMode는 새로 설정할 카메라 모드
CCamera* CAirplanePlayer::ChangeCamera(DWORD dwNewCameraMode, FLOAT fTimeElapsed)
{
	DWORD nCurrentCameraMode{ (m_pCamera) ? m_pCamera->GetMode() : 0x00 };

	if (nCurrentCameraMode == dwNewCameraMode)
		return m_pCamera;
	...
	case FIRST_PERSON_CAMERA:
	{
		...
		m_pCamera = ChangeCamera(FIRST_PERSON_CAMERA, nCurrentCameraMode);
	...
	case SPACESHIP_CAMERA:
	{
		...
		m_pCamera = ChangeCamera(SPACESHIP_CAMERA, nCurrentCameraMode);
	...
	case THIRD_PERSON_CAMERA:
	{
		...
		m_pCamera = ChangeCamera(THIRD_PERSON_CAMERA, nCurrentCameraMode);
	...
	// 플레이어를 시간의 경과에 따라 갱신(위치와 방향을 변경: 속도, 마찰력, 중력 등 처리)
	Update(fTimeElapsed);

	return m_pCamera;
}

void CAirplanePlayer::PrepareRender()
{
	...
	왜냐하면 비행기 모델 Mesh는 y축 방향이 비행기의 앞쪽이 되도록 모델링이 되었기 때문이다.
	그리고 이 Mesh를 카메라의 z축 방향으로 향하도록 그릴 것이기 때문이다.*/
}
