...
void CGameFramework::BuildObjects()
{
	...
	// 카메라 객체를 생성하여 Viewport, Scissor 사각형, 투영 변환 행렬, 카메라 변환 행렬을 생성 및 설정
...
void CGameFramework::ProcessInput()
{
	...
	/*키보드의 상태 정보를 반환. 화살표 키(‘→’, ‘←’, ‘↑’, ‘↓’)를 누르면 플레이어를 오른쪽/왼쪽(로컬 x축), 앞/뒤(로컬 z축)로 이동.
	...
	/*마우스를 캡쳐했으면 마우스가 얼마만큼 이동하였는 가를 계산.
	마우스 왼쪽 또는 오른쪽 버튼이 눌러질 때의 메시지(WM_LBUTTONDOWN, WM_RBUTTONDOWN)를 처리할 때 마우스 캡쳐.
	그러므로 마우스가 캡쳐된 것은 마우스 버튼이 눌려진 상태를 의미.
	...
	if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f))
	{
		...
		/*플레이어를 dwDirection 방향으로 이동(실제로는 속도 벡터 변경). 이동 거리는 시간에 비례하도록 설정.
		플레이어의 이동 속력은 (50/초)로 가정.*/
...
{
	// 타이머의 시간이 갱신되도록 하고 Frame Rate를 계산
	...
	/*현재 Render Target에 대한 프리젠트가 끝나기를 기다린다. 프리젠트가 끝나면 Render Target 버퍼의 상태는 프리젠트 상태
	(D3D12_RESOURCE_STATE_PRESENT)에서 Render Target 상태(D3D12_RESOURCE_STATE_RENDER_TARGET)로 바뀔 것이다.*/
	m_pd3dCommandList->ResourceBarrier(1, &d3dResourceBarrier);
	
	D3D12_CPU_DESCRIPTOR_HANDLE d3dRtvCPUDescriptorHandle{ m_pd3dRtvDescriptorHeap->GetCPUDescriptorHandleForHeapStart() };

	// 현재의 Render Target에 해당하는 서술자의 CPU 주소(핸들) 계산
	d3dRtvCPUDescriptorHandle.ptr += (m_nSwapChainBufferIndex *	m_nRtvDescriptorIncrementSize);

	FLOAT pfClearColor[4] = { 0.0f, 0.125f, 0.3f, 1.0f };
	// 원하는 색상으로 Render Target(뷰)을 지운다.
	m_pd3dCommandList->ClearRenderTargetView(d3dRtvCPUDescriptorHandle, pfClearColor/*Colors::Azure*/, 0, nullptr);

	// Depth-Stencil 서술자의 CPU 주소를 계산한다.
	D3D12_CPU_DESCRIPTOR_HANDLE d3dDsvCPUDescriptorHandle{ m_pd3dDsvDescriptorHeap->GetCPUDescriptorHandleForHeapStart() };
	
	// 원하는 값으로 Depth-Stencil(뷰)을 지운다.
	...
	// Render Target View(서술자)와 Depth-Stencil View(서술자)를 출력-병합 단계(OM)에 연결
	m_pd3dCommandList->OMSetRenderTargets(1, &d3dRtvCPUDescriptorHandle, TRUE, &d3dDsvCPUDescriptorHandle);

	// Rendering 코드는 여기에 추가될 것이다.
	if (m_pScene)
		m_pScene->Render(m_pd3dCommandList, m_pCamera);

	// 3인칭 카메라일 때 플레이어가 항상 보이도록 Rendering
#ifdef _WITH_PLAYER_TOP
	// Render Target은 그대로 두고 Depth 버퍼를 1.0으로 지우고 플레이어를 Rendering하면 플레이어는 무조건 그려질 것이다.
	m_pd3dCommandList->ClearDepthStencilView(d3dDsvCPUDescriptorHandle, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0, nullptr);
#endif
	
	//3인칭 카메라일 때 플레이어를 Rendering
	if (m_pPlayer)
		m_pPlayer->Render(m_pd3dCommandList, m_pCamera);

	/*현재 Render Target에 대한 Rendering이 끝나기를 기다린다. GPU가 Render Target(버퍼)을 더 이상 사용하지 않으면 Render Target의
	...
	// 명령 List를 명령 Queue에 추가하여 실행
	ID3D12CommandList* ppd3dCommandLists[] = { m_pd3dCommandList };
	m_pd3dCommandQueue->ExecuteCommandLists(_countof(ppd3dCommandLists), ppd3dCommandLists);
	
	// GPU가 모든 명령 List를 실행할 때 까지 기다린다.
	WaitForGpuComplete();
	
	// Swap Chain을 프리젠트한다. 프리젠트를 하면 현재 Render Target(후면버퍼)의 내용이 전면버퍼로 옮겨지고 Render Target 인덱스가 바뀔 것이다.
...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		case VK_F8:
			break;
		case VK_F9:						// F9키가 눌려지면 윈도우 모드와 전체화면 모드의 전환을 처리
		{
